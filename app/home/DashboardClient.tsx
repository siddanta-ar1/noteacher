...
// Types matching the data we pass from the Server
type DashboardProps = {
  profile: {
    xp: number;
    name: string;
  ...
  courses: Array<{
    ...
    progress: number;
    color: string;
    iconName: "Zap" | "BookOpen" | "Trophy" | "Star";
  }>;
};
        ...
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/leaderboard">
              <button className="bg-white/10 backdrop-blur-md w-10 h-10 rounded-2xl flex items-center justify-center border border-white/20 hover:bg-white/20 transition-colors">
            ...
            {activeMission ? (
              <>
                ...
                <Link href={`/lesson/${activeMission.id}`}>
                  <button className="px-10 py-4 bg-navy text-white rounded-2xl font-black border-b-4 border-navy-dark active:translate-y-1 active:border-b-0 transition-all shadow-lg hover:shadow-xl">
                    Resume <Play className="inline ml-2 w-5 h-5 fill-white" />
                  </button>
                </Link>
              </>
            ) : (
              // Fallback if no active mission (Course Complete or Not Started)
            ...
            {courses.map((course) => {
              ...
              return (
                <Link href={`/courses`} key={course.id}>
                  <div className="bg-white border-2 border-slate-100 p-6 rounded-[2rem] hover:shadow-xl transition-all cursor-pointer group h-full flex flex-col">
                    <div
